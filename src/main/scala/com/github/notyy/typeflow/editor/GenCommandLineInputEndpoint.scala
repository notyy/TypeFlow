...
class GenCommandLineInputEndpoint(val model: Model) {
  ...
  def execute(packageName: PackageName, commandLineInputEndpoint: CommandLineInputEndpoint, codeTemplate: CodeTemplate): ScalaCode = {
    val code = codeTemplate.value.replaceAllLiterally("$PackageName$", packageName.value).
      ...
      replaceAllLiterally("$CallingChain$", accuStatements(flow.instances.find(_.id == commandLineInputEndpoint.name).get, 1, "input", flow.connections, Vector.empty).mkString(System.lineSeparator()))
    ScalaCode(s"${packageName.value}.${commandLineInputEndpoint.name}", code)
  }

  def accuStatements(outFrom: Instance, outputIndex: Int, outputParamName: String, connections: Vector[Connection], currStatements: Vector[String]): Vector[String] = {
    ...
    if (connsFromThis.isEmpty) {
      ...
      connsFromThis.flatMap { conn =>
        ...
        val targetInstance = flow.instances.find(_.id == targetInstanceId).get
        ...
        if (targetOutputs.size == 1) {
          ...
          if (statement.isDefined) {
            accuStatements(targetInstance, 1, resultName, connections, currStatements.appended(statement.get))
          } else {
            currStatements
          }
        } else {
